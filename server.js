...
<script>
var socket=io(),state=null,currentTab="overview",editingQuestionId=null,resultIndex=0,pendingTab=null,podiumStep=0,displayView="auto";
...
function send(eventName,goTab){if(goTab){pendingTab=goTab;displayView=goTab}if(eventName==="adminReleaseResults"||eventName==="modReleaseResults"){resultIndex=0;displayView="results"}socket.emit(eventName)}
...
function resultSlide(isDisplay){if(!state.questionResults.length)return el("div","card","Noch keine Fragen vorhanden.");if(resultIndex<0)resultIndex=0;if(resultIndex>=state.questionResults.length)resultIndex=state.questionResults.length-1;var q=state.questionResults[resultIndex],card=el("div","card screen"+(isDisplay?" displayCard":"")),inner=el("div");inner.style.width="100%";inner.appendChild(el("div","pill status","Auflösung · Frage "+(resultIndex+1)+" von "+state.questionResults.length));inner.appendChild(el("h1","",q.text));inner.appendChild(el("div","huge",q.answerLabel+" "+(q.unit||"")));if(q.explanation)inner.appendChild(el("p","sub",q.explanation));inner.appendChild(barChart(q));var nav=el("div","row displayActions"),prev=el("button","btn ghost","Vorherige Frage");prev.disabled=resultIndex===0;prev.onclick=function(){resultIndex--;render()};nav.appendChild(prev);if(resultIndex<state.questionResults.length-1){var next=el("button","btn alt","Nächste Frage");next.onclick=function(){resultIndex++;render()};nav.appendChild(next)}else if(state.quizStatus==="released"){nav.appendChild(action("Siegerehrung freigeben","green",isDisplay?"modReleasePodium":"adminReleasePodium","podium"))}if(state.quizStatus==="podium"){var pod=el("button","btn green","Zur Siegerehrung");pod.onclick=function(){displayView="podium";currentTab="podium";render()};nav.appendChild(pod)}inner.appendChild(nav);card.appendChild(inner);return card}
function podiumView(rows){var places=[rows.filter(function(r){return r.rank===2}),rows.filter(function(r){return r.rank===1}),rows.filter(function(r){return r.rank===3})];var labels=[["🥈","Platz 2","second"],["🥇","Platz 1","first"],["🥉","Platz 3","third"]];var podium=el("div","podium");labels.forEach(function(label,i){var card=el("div","podiumCard "+label[2]);card.appendChild(el("div","medal",label[0]));card.appendChild(el("h2","",label[1]));if(!places[i].length)card.appendChild(el("p","muted","-"));places[i].forEach(function(r){card.appendChild(el("div","podiumName",r.teamLabel));card.appendChild(el("p","big",r.score+" Punkte"))});podium.appendChild(card)});return podium}
function renderPodiumPage(root){clear(root);if(state.quizStatus!=="podium"){var w=el("div","card screen"),i=el("div");i.appendChild(el("div","pill status","Siegerehrung"));i.appendChild(el("h2","","Die Siegerehrung wurde noch nicht freigegeben."));i.appendChild(el("p","sub","Nach der Freigabe erscheinen hier ausschließlich die Top 3."));w.appendChild(i);root.appendChild(w);return}root.appendChild(podiumSlide(false))}
function podiumSlide(isDisplay){var c=el("div","card screen"+(isDisplay?" displayCard":"")),i=el("div");i.style.width="100%";var titles=["Jetzt kommt die Siegerehrung","Platz 3","Platz 2","Platz 1","Unsere Gewinnerteams","Gesamtrangliste"];i.appendChild(el("div","pill status","Siegerehrung"));i.appendChild(el("h1","",titles[podiumStep]||titles[0]));if(podiumStep===0)i.appendChild(el("p","sub","Gleich enthüllen wir die Top 3."));if(podiumStep>0&&podiumStep<4)i.appendChild(singlePlace([3,2,1][podiumStep-1]));if(podiumStep===4||podiumStep===5)i.appendChild(podiumView(state.podium));if(podiumStep===5)i.appendChild(table(["Platz","Team/Symbol","Punkte"],state.leaderboard.map(function(r){return["#"+r.rank,r.teamLabel,String(r.score)]})));var nav=el("div","row displayActions");var back=el("button","btn ghost","Zurück zur Auflösung");back.onclick=function(){displayView="results";currentTab="results";render()};nav.appendChild(back);if(podiumStep>0){var prev=el("button","btn ghost","Zurück");prev.onclick=function(){podiumStep--;render()};nav.appendChild(prev)}if(podiumStep<4){var next=el("button","btn alt","Weiter");next.onclick=function(){podiumStep++;render()};nav.appendChild(next)}else if(podiumStep===4){var list=el("button","btn green","Gesamtrangliste anzeigen");list.onclick=function(){podiumStep=5;render()};nav.appendChild(list)}else{var restart=el("button","btn alt","Siegerehrung neu starten");restart.onclick=function(){podiumStep=0;render()};nav.appendChild(restart)}i.appendChild(nav);c.appendChild(i);return c}
function singlePlace(rank){var rows=state.podium.filter(function(r){return r.rank===rank});var wrap=el("div","podium");var card=el("div","podiumCard "+(rank===1?"first":rank===2?"second":"third"));card.appendChild(el("div","medal",rank===1?"🥇":rank===2?"🥈":"🥉"));card.appendChild(el("h2","","Platz "+rank));if(!rows.length)card.appendChild(el("p","muted","-"));rows.forEach(function(r){card.appendChild(el("div","podiumName",r.teamLabel));card.appendChild(el("p","big",r.score+" Punkte"))});wrap.appendChild(card);return wrap}
