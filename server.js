...
function renderReleased(state) { var root = qs("#game"); clear(root); var card = el("div", "card"); card.appendChild(el("div", "pill status", state.quizStatus === "podium" ? "Siegerehrung freigegeben" : "Ergebnisse freigegeben")); card.appendChild(el("h2", "", "Auflösung")); card.appendChild(el("p", "muted", "Hier seht ihr die richtigen Antworten und eure Abstände. Platzierungen werden erst in der Siegerehrung sichtbar.")); state.questionDetails.forEach(function(q, index) { var item = el("div", "item"); item.appendChild(el("h3", "", "Frage " + (index + 1) + ": " + q.text)); item.appendChild(el("p", "", "Richtige Antwort: " + q.answerLabel + " " + (q.unit || ""))); item.appendChild(el("p", "", "Eure Schätzung: " + q.ownValueLabel + " " + (q.unit || "") + " · Abstand: " + q.ownDiffLabel + " " + (q.unit || ""))); if (q.explanation) item.appendChild(el("p", "muted", q.explanation)); item.appendChild(distributionBlock(q)); card.appendChild(item); }); if (state.quizStatus === "podium" && state.podium.length) { card.appendChild(el("h2", "", "Top 3")); card.appendChild(podiumView(state.podium)); } root.appendChild(card); }
function podiumView(rows) { var places = [rows.filter(function(row) { return row.rank === 2; }), rows.filter(function(row) { return row.rank === 1; }), rows.filter(function(row) { return row.rank === 3; })]; var labels = [["🥈", "Platz 2", "second"], ["🥇", "Platz 1", "first"], ["🥉", "Platz 3", "third"]]; var podium = el("div", "podium"); labels.forEach(function(label, i) { var card = el("div", "podiumCard " + label[2]); card.appendChild(el("div", "medal", label[0])); card.appendChild(el("h2", "", label[1])); if (!places[i].length) card.appendChild(el("p", "muted", "-")); places[i].forEach(function(row) { card.appendChild(el("div", "podiumName", row.teamLabel)); card.appendChild(el("p", "big", row.score + " Punkte")); }); card.appendChild(el("div", "stepBlock", label[1])); podium.appendChild(card); }); return podium; }
...
function actionButton(text, className, eventName, confirmText) { var button = el("button", "btn " + className, text); button.onclick = function() { if (confirmText && !confirm(confirmText)) return; socket.emit(eventName); }; return button; }
...
function distributionBlock(question) { var wrap = el("div", "item"); var values = question.distribution.map(function(row) { return row.value; }).concat([question.answer]); var min = Math.min.apply(null, values); var max = Math.max.apply(null, values); if (min === max) { min = min - 1; max = max + 1; } var correctPct = ((question.answer - min) / (max - min)) * 100; question.distribution.forEach(function(row) { var pct = ((row.value - min) / (max - min)) * 100; var line = el("div", "distRow"); line.appendChild(el("div", "", row.teamLabel)); line.appendChild(el("div", "", row.valueLabel + " " + (question.unit || ""))); var bar = el("div", "distBar"); var correct = el("div", "distCorrect"); correct.style.left = correctPct + "%"; var dot = el("div", "distDot"); dot.style.left = "calc(" + pct + "% - 6px)"; bar.appendChild(correct); bar.appendChild(dot); line.appendChild(bar); line.appendChild(el("div", "", "Abstand: " + row.diffLabel)); wrap.appendChild(line); }); var legend = el("p", "muted", "Linie = richtige Antwort, Punkt = Schätzung. Sortiert nach Schätzwert, nicht nach Platzierung."); wrap.appendChild(legend); return wrap; }
function renderResults() { var root = qs("#tab-results"); clear(root); if (state.quizStatus !== "released" && state.quizStatus !== "podium") { var card = el("div", "card screen"); var inner = el("div"); inner.appendChild(el("div", "pill status", statusLabel(state.quizStatus))); inner.appendChild(el("h2", "", "Die Ergebnisse sind noch nicht freigegeben.")); inner.appendChild(el("p", "sub", "Hier erscheinen später nur die Auflösungen und Antwortverteilungen — keine Punkte und keine Platzierungen.")); card.appendChild(inner); root.appendChild(card); return; } state.questionResults.forEach(function(question, index) { var card = el("div", "card"); card.style.marginTop = "16px"; card.appendChild(el("h2", "", "Frage " + (index + 1))); card.appendChild(el("h3", "", question.text)); card.appendChild(el("p", "", "Richtige Antwort: " + question.answerLabel + " " + (question.unit || ""))); if (question.explanation) card.appendChild(el("p", "muted", question.explanation)); card.appendChild(distributionBlock(question)); card.appendChild(table(["Team/Symbol", "Schätzung", "Abstand"], question.distribution.map(function(row) { return [row.teamLabel, row.valueLabel + " " + (question.unit || ""), row.diffLabel + " " + (question.unit || "")]; }))); root.appendChild(card); }); }
