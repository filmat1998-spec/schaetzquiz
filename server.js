...
.confettiLayer{position:absolute;inset:0;z-index:4;pointer-events:none;overflow:hidden;border-radius:30px;animation:confettiLayerFade 3s ease forwards}.confettiPiece{position:absolute;top:-40px;width:12px;height:18px;border-radius:4px;opacity:.95;animation:confettiFall 4.2s cubic-bezier(.18,.72,.28,1) forwards}.confettiPiece:nth-child(3n){width:10px;height:10px;border-radius:50%}.confettiPiece:nth-child(4n){width:16px;height:8px}.confettiPiece:nth-child(5n){height:24px}@keyframes confettiFall{0%{transform:translate3d(0,-40px,0) rotate(0deg)}100%{transform:translate3d(var(--drift),120vh,0) rotate(900deg)}}@keyframes confettiLayerFade{0%,72%{opacity:1}100%{opacity:0}}
...
function handleParticipantState(s){var input=qs("#answerInput");if(input&&currentState&&currentState.quizStatus==="open"&&s.quizStatus==="open"&&currentState.nextQuestion&&s.nextQuestion&&currentState.nextQuestion.id===s.nextQuestion.id){currentState=s;return}render(s)}
var storedTeam=localStorage.getItem(teamStorageKey);if(storedTeam)qs("#teamLabel").value=storedTeam;qs("#joinBtn").onclick=join;qs("#teamLabel").addEventListener("keydown",function(e){if(e.key==="Enter")join()});socket.on("participantAccepted",function(d){localStorage.setItem(teamStorageKey,d.teamLabel)});socket.on("participantReset",function(){localStorage.removeItem(teamStorageKey);location.reload()});socket.on("participantError",toast);socket.on("participantState",handleParticipantState);
</script>
</body></html>`;
}
